<script lang="ts">
  ...
  import SentimentCalendar from "$lib/components/stats/SentimentCalendar.svelte";
  ...
  const shareResult = async () => {
    ...
    const shareTextReplaced = shareTextBase
      ...
      .replace(
        "${randomAdjective}",
        document.getElementById("randomAdjective")?.innerText || "Unknown",
      )
      .replace(
        "${randomNounEmoji}",
        document.getElementById("randomNounEmoji")?.innerText || "User",
      );
  ...
  // Util
  // ------------------------
  function getPeakFriendsActivityHour(
    resultAnalyze: App.ResultAnalyze,
  ): number | null {
  ...
  function isValidPercentiles(obj: any): obj is App.Percentiles {
    return (
      obj &&
      requiredKeys.every((key) => key in obj && typeof obj[key] === "number")
    );
  }
</script>
...
{:else if resultAnalyze}
  <div class="p-8 bg-gray-100 space-y-4">
    ...
    <div class="flex items-center justify-between mb-6 flex-wrap">
      <h2 class="w-3/4 text-2xl font-bold text-gray-700 break-words">
        @{handle}'s Stats in Bluesky
      </h2>
      <!-- シェアボタン -->
      <button
        onclick={shareResult}
    ...
    <div class="space-y-4">
      ...
      {#if resultAnalyze.activity.post.sentimentHeatmap}
        <LineGraph
          data={resultAnalyze.activity.post.sentimentHeatmap}
          centerZero={true}
        />
      {/if}

      {#if resultAnalyze.activity.post.sentimentCalendar}
        <SentimentCalendar
          data={resultAnalyze.activity.post.sentimentCalendar}
        />
      {/if}
          ...
          cards={[
            {
              ...
              title: "Avg Interval",
              content:
                resultAnalyze.activity.post.reply.averageInterval || null,
